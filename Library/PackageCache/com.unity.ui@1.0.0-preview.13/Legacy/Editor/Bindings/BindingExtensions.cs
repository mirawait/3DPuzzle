...
namespace UnityEditor.UIElements.Bindings
{
    internal class SerializedObjectBindingContext
    {
        ...
        internal void BindTree(VisualElement element, SerializedProperty parentProperty)
        {
            if (ShouldDelayBind())
            {
                ...
                var request = LegacySerializedObjectBindindImplementation.BindingRequest.CreateDelayBinding(this, parentProperty);
    ...
    [InitializeOnLoad]
    internal class LegacySerializedObjectBindindImplementation : ISerializedObjectBindingImplementation
    {
        static LegacySerializedObjectBindindImplementation()
        {
            BindingExtensions.bindingImpl = new LegacySerializedObjectBindindImplementation();
        }
    ...
    class SerializedObjectBinding<TValue> : SerializedObjectBindingPropertyToBaseField<TValue, TValue>
    {
            ...
            Func<TValue, SerializedProperty, Func<SerializedProperty, TValue>, bool> compareValues)
        {
            ...
            Update();
#if UNITY_2020_2_OR_NEWER
            // Since the editor assembly is not overridable in 2020.1, we can't have that fix since that PropertyField will loop infinitely
