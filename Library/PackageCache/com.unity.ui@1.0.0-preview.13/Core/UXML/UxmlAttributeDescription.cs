            ...
            if (name == null)
            {
                ...
                for (var i = 0; i < m_ObsoleteNames.Length; i++)
                {
                    if (bag.TryGetAttributeValue(m_ObsoleteNames[i], out value))
                    {
                        ...
                        {
#if ENABLE_API_CHANGES_WARNING
                            Debug.LogWarningFormat(cc.visualTreeAsset, "In {0}.uxml: '{1}' attribute is deprecated. Please remove it.", cc.visualTreeAsset.name, obsoleteNames[i], name);
#endif
                        }
                ...
                if (m_ObsoleteNames != null)
                {
                    for (var i = 0; i < m_ObsoleteNames.Length; i++)
                    {
                        if (bag.TryGetAttributeValue(m_ObsoleteNames[i], out value))
                        {
                            ...
                            {
#if ENABLE_API_CHANGES_WARNING
                                Debug.LogWarningFormat(cc.visualTreeAsset, "In {0}.uxml: '{1}' attribute name is deprecated. Please use '{2}' instead.", cc.visualTreeAsset.name, obsoleteNames[i], name);
#endif
                            }
