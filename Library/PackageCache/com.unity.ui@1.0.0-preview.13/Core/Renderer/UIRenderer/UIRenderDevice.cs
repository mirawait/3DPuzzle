...
namespace UnityEngine.UIElements.UIR
{
    ...
    internal class UIRenderDevice : IDisposable
    {
        ...
        {
            public uint allocAttempts;
            public uint allocCount, vertexAllocSize, indexAllocSize;
            public uint freeCount, vertexFreeSize, indexFreeSize;
            public uint newPageCount;
        }
        DiagnosticStats m_DiagStats;
#endif
...
#if RD_DIAGNOSTICS
            m_DiagStats.allocCount++;
            m_DiagStats.vertexAllocSize += vertexCount;
            m_DiagStats.indexAllocSize += indexCount;
#endif
...
#if UNITY_UIR_DEBUG
            meshHandle.device = this; // Track device per mesh handle for debugging
#endif
...
#if RD_DIAGNOSTICS
            m_DiagStats.freeCount++;
            m_DiagStats.vertexFreeSize += meshObj.allocVerts.size;
            m_DiagStats.indexFreeSize += meshObj.allocIndices.size;
#endif
        }
...
#if RD_DIAGNOSTICS
            m_DiagStats = new DiagnosticStats(); // Reset stats
#endif
        }
