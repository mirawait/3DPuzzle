...
namespace UnityEngine.TextCore.Text
{
    ...
    internal class TextGenerator
    {
        ...
        //private Character m_CachedEllipsisGlyphInfo = null;
        ...
        void GenerateTextMesh(TextGenerationSettings generationSettings, TextInfo textInfo)
        {
            ...
            for (int i = 0; i < m_CharBuffer.Length && m_CharBuffer[i] != 0; i++)
            {
                ...
                if (generationSettings.enableKerning)
                {
                    ...
                    uint baseGlyphIndex = m_CachedTextElement.glyphIndex;

                    if (m_CharacterCount < totalCharacterCount - 1)
                    {
                        uint nextGlyphIndex = textInfo.textElementInfo[m_CharacterCount + 1].textElement.glyphIndex;
                    ...
                    {
                        uint previousGlyphIndex = textInfo.textElementInfo[m_CharacterCount - 1].textElement.glyphIndex;
                ...
                else
                {
                    ...
                    {
                        float gradientScale = m_CurrentMaterial.GetFloat(TextShaderUtilities.ID_GradientScale);
                        stylePadding = m_CurrentFontAsset.regularStyleWeight / 4.0f * gradientScale * m_CurrentMaterial.GetFloat(TextShaderUtilities.ID_ScaleRatio_A);
                        ...
                        switch (generationSettings.overflowMode)
                        {
                            ...
                            case TextOverflowMode.Ellipsis:
                                ...
                                if (m_Ellipsis.character != null)
                                {
                                    ...
                                    textInfo.textElementInfo[ellipsisIndex].textElement = m_Ellipsis.character;
                ...
                if (generationSettings.wordWrap || generationSettings.overflowMode == TextOverflowMode.Truncate || generationSettings.overflowMode == TextOverflowMode.Ellipsis)
                {
                    ...
                    {
                        if (isFirstWord || isLastBreakingChar || textSettings.lineBreakingRules.leadingCharactersLookup.Contains((uint)charCode) == false &&
                            (m_CharacterCount < totalCharacterCount - 1 &&
                             textSettings.lineBreakingRules.followingCharactersLookup.Contains(textInfo.textElementInfo[m_CharacterCount + 1].character) == false))
                        {
                            SaveWordWrappingState(ref m_SavedWordWrapState, i, m_CharacterCount, textInfo);
                            m_IsCharacterWrappingEnabled = false;
                            isFirstWord = false;
        ...
        bool ValidateRichTextTag(string sourceText, ref int readIndex, ref int writeIndex, TextGenerationSettings generationSettings, TextInfo textInfo)
        {
            ...
            for (; readIndex < srcLength && sourceText[readIndex] != 0 /* && tagCharCount < s_RichTextTag.Length */; readIndex++)
            {
                ...
                if (readingFlag == 0)
                {
                    ...
                    if (c == '#')
                    {
                        m_Attributes[attributeIndex].nameHashCode = (int)MarkupTag.COLOR;
        ...
        protected bool ValidateHtmlTag(int[] chars, int startIndex, out int endIndex, TextGenerationSettings generationSettings, TextInfo textInfo)
        {
            ...
            if (m_TagNoParsing && ((MarkupTag)m_XmlAttribute[0].nameHashCode != MarkupTag.SLASH_NO_PARSE))
            {
                return false;
            }
            if ((MarkupTag)m_XmlAttribute[0].nameHashCode == MarkupTag.SLASH_NO_PARSE)
            {
                m_TagNoParsing = false;
                return true;
            }
            ...
            else
            {
                ...
                switch ((MarkupTag)m_XmlAttribute[0].nameHashCode)
                {
                    ...
                    case MarkupTag.SLASH_STRIKETHROUGH: // </s>
                    ...
                    case MarkupTag.SLASH_UNDERLINE: // </u>
                    ...
                    case MarkupTag.MARK: // <mark=#FF00FF80>
                    ...
                    case MarkupTag.SLASH_MARK: // </mark>
                    ...
                    case MarkupTag.SLASH_SUBSCRIPT: // </sub>
                    ...
                    case MarkupTag.SUPERSCRIPT: // <sup>
                    ...
                    case MarkupTag.SLASH_SUPERSCRIPT: // </sup>
                    ...
                    case MarkupTag.FONT_WEIGHT: // <font-weight>
                    ...
                    case MarkupTag.SLASH_FONT_WEIGHT: // </font-weight>
                    ...
                    case MarkupTag.POSITION: // <pos=000.00px> <pos=0em> <pos=50%>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                    ...
                    case MarkupTag.SLASH_POSITION: // </pos>
                        // No affect.
                        return true;


                    case MarkupTag.VERTICAL_OFFSET: // <voffset>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_VERTICAL_OFFSET: // </voffset>
                        m_BaselineOffset = 0;
                        return true;

                    case MarkupTag.PAGE: // <page>
                    ...
                    case MarkupTag.NO_BREAK: // <nobr>
                        m_IsNonBreakingSpace = true;
                        return true;

                    case MarkupTag.SLASH_NO_BREAK: // </nobr>
                        m_IsNonBreakingSpace = false;
                        return true;

                    case MarkupTag.SIZE: // <size=>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_SIZE: // </size>
                        m_CurrentFontSize = m_SizeStack.Remove();
                        m_FontScale = (m_CurrentFontSize / m_CurrentFontAsset.faceInfo.pointSize * m_CurrentFontAsset.faceInfo.scale);
                        return true;

                    case MarkupTag.FONT: // <font=xx>
                        ...
                        else if (materialAttributeHashCode == (uint)MarkupTag.MATERIAL) // using material attribute
                        {
                    ...
                    case MarkupTag.SLASH_FONT: // </font>
                    {
                    ...
                    case MarkupTag.MATERIAL: // <material="material name">
                    ...
                    case MarkupTag.SLASH_MATERIAL: // </material>
                    {
                        MaterialReference materialReference = m_MaterialReferenceStack.Remove();
                    ...
                    case MarkupTag.SPACE: // <space=000.00>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.A: // <a href=" ">
                        return false;

                    case MarkupTag.SLASH_A: // </a>
                        return true;

                    case MarkupTag.LINK: // <link="name">
                    ...
                    case MarkupTag.SLASH_LINK: // </link>
                        if (m_IsParsingText && !m_IsCalculatingPreferredValues)
                        {
                    ...
                    case MarkupTag.SLASH_ALIGN: // </align>
                        m_LineJustification = m_LineJustificationStack.Remove();
                        return true;

                    case MarkupTag.WIDTH: // <width=xx>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_WIDTH: // </width>
                        m_Width = -1;
                        return true;

                    case MarkupTag.COLOR: // <color> <color=#FF00FF> or <color=#FF00FF00>
                        ...
                        switch ((MarkupTag)m_XmlAttribute[0].valueHashCode)
                        {
                            case MarkupTag.RED: // <color=red>
                                m_HtmlColor = Color.red;
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.BLUE: // <color=blue>
                                m_HtmlColor = Color.blue;
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.BLACK: // <color=black>
                                m_HtmlColor = Color.black;
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.GREEN: // <color=green>
                                m_HtmlColor = Color.green;
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.WHITE: // <color=white>
                                m_HtmlColor = Color.white;
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.ORANGE: // <color=orange>
                                m_HtmlColor = new Color32(255, 128, 0, 255);
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.PURPLE: // <color=purple>
                                m_HtmlColor = new Color32(160, 32, 240, 255);
                                m_ColorStack.Add(m_HtmlColor);
                                return true;
                            case MarkupTag.YELLOW: // <color=yellow>
                    ...
                    case MarkupTag.SLASH_COLOR: // </color>
                        m_HtmlColor = m_ColorStack.Remove();
                        return true;

                    case MarkupTag.GRADIENT: //<gradient>
                        ...
                        else
                        {
                            ...
                            {
                                tempColorGradientPreset = Resources.Load<TextColorGradient>(textSettings.defaultColorGradientPresetsPath + new string(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength));
                            }
                    ...
                    case MarkupTag.SLASH_GRADIENT: // </gradient>
                        m_ColorGradientPreset = m_ColorGradientStack.Remove();
                        return true;

                    case MarkupTag.CHARACTER_SPACE: // <cspace=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_CHARACTER_SPACE: // </cspace>
                        if (!m_IsParsingText)
                            return true;
                    ...
                    case MarkupTag.MONOSPACE: // <mspace=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_MONOSPACE: // </mspace>
                        m_MonoSpacing = 0;
                        return true;

                    case MarkupTag.CLASS: // <class="name">
                        return false;

                    case MarkupTag.INDENT: // <indent=10px> <indent=10em> <indent=50%>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_INDENT: // </indent>
                        m_TagIndent = m_IndentStack.Remove();
                        return true;

                    case MarkupTag.LINE_INDENT: // <line-indent>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_LINE_INDENT: // </line-indent>
                        m_TagLineIndent = 0;
                        return true;

                    case MarkupTag.SPRITE: // <sprite=x>
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    tempSpriteAsset = Resources.Load<SpriteAsset>(textSettings.defaultSpriteAssetPath + new string(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength));
                                }
                        ...
                        for (int i = 0; i < m_XmlAttribute.Length && m_XmlAttribute[i].nameHashCode != 0; i++)
                        {
                            ...
                            {
                                case MarkupTag.NAME: // <sprite name="">
                                    ...
                                    m_SpriteIndex = index;
                                    break;
                                case MarkupTag.INDEX: // <sprite index=>
                                ...
                                case MarkupTag.TINT: // tint
                                    ...
                                    break;
                                case MarkupTag.COLOR: // color=#FF00FF80
                                    ...
                                    break;
                                case MarkupTag.ANIM: // anim="0,16,12"  start, end, fps
                    ...
                    case MarkupTag.ALLCAPS: // <allcaps>
                    case MarkupTag.UPPERCASE: // <uppercase>
                        m_FontStyleInternal |= FontStyles.UpperCase;
                        m_FontStyleStack.Add(FontStyles.UpperCase);
                        return true;

                    case MarkupTag.SLASH_ALLCAPS: // </allcaps>
                    case MarkupTag.SLASH_UPPERCASE: // </uppercase>
                    ...
                    case MarkupTag.MARGIN: // <margin=00.0> <margin=00em> <margin=50%>
                    ...
                    case MarkupTag.SLASH_MARGIN: // </margin>
                        m_MarginLeft = 0;
                        m_MarginRight = 0;
                        return true;

                    case MarkupTag.MARGIN_LEFT: // <margin-left=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength); // px
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.MARGIN_RIGHT: // <margin-right=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength); // px
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.LINE_HEIGHT: // <line-height=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                    ...
                    case MarkupTag.SLASH_LINE_HEIGHT: // </line-height>
                        m_LineHeight = k_FloatUnset;
                        return true;

                    case MarkupTag.NO_PARSE: // <noparse>
                        m_TagNoParsing = true;
                        return true;

                    case MarkupTag.ACTION: // <action>
                    ...
                    case MarkupTag.SLASH_ACTION: // </action>
                        m_ActionStack.Remove();
                        return false;

                    case MarkupTag.SCALE: // <scale=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_SCALE: // </scale>
                        m_IsFxMatrixSet = false;
                        return true;

                    case MarkupTag.ROTATE: // <rotate=xx.x>
                        value = TextGeneratorUtilities.ConvertToFloat(m_RichTextTag, m_XmlAttribute[0].valueStartIndex, m_XmlAttribute[0].valueLength);
                        if (value == k_FloatUnset)
                            return false;
                    ...
                    case MarkupTag.SLASH_ROTATE: // </rotate>
                        m_IsFxMatrixSet = false;
                        return true;
                }
            }
        ...
        void DrawUnderlineMesh(Vector3 start, Vector3 end, ref int index, float startScale, float endScale, float maxScale, float sdfScale, Color32 underlineColor, TextGenerationSettings generationSettings, TextInfo textInfo)
        {
            ...
            // UNDERLINE UV0
            Vector2[] uvs0 = textInfo.meshInfo[underlineMaterialIndex].uvs0;
        ...
        void SetArraySizes(int[] chars, TextGenerationSettings generationSettings, TextInfo textInfo)
        {
            ...
            for (int i = 0; i < chars.Length && chars[i] != 0; i++)
            {
                ...
                if (character == null)
                {
                    ...
                    unicode = textSettings.missingCharacterUnicode == 0 ? 9633 : textSettings.missingCharacterUnicode;
                    ...
                    if (character == null)
                    {
                        ...
                        if (textSettings.displayWarnings)
                        {
                            string formattedWarning = srcGlyph > 0xFFFF
                                ? string.Format("The character with Unicode value \\U{0:X8} was not found in the [{1}] font asset or any potential fallbacks. It was replaced by a space.", srcGlyph, generationSettings.fontAsset.name)
                                : string.Format("The character with Unicode value \\u{0:X4} was not found in the [{1}] font asset or any potential fallbacks. It was replaced by a space.", srcGlyph, generationSettings.fontAsset.name);
                ...
                if (character.elementType == TextElementType.Sprite)
                {
                    ...
                    textInfo.textElementInfo[m_TotalCharacterCount].materialReferenceIndex = m_CurrentMaterialIndex;
                    textInfo.textElementInfo[m_TotalCharacterCount].spriteAsset = spriteAssetRef;
                    textInfo.textElementInfo[m_TotalCharacterCount].spriteIndex = (int)character.glyphIndex;
            ...
            for (int i = 0; m_CharBuffer[i] != 0; i++)
            {
                ...
                if (generationSettings.wordWrap || generationSettings.overflowMode == TextOverflowMode.Truncate || generationSettings.overflowMode == TextOverflowMode.Ellipsis)
                {
                             ...
                             && !m_IsNonBreakingSpace)
                    {
                        if (isFirstWord || isLastBreakingChar || textSettings.lineBreakingRules.leadingCharactersLookup.Contains((uint)charCode) == false &&
                            (m_CharacterCount < totalCharacterCount - 1 &&
                             textSettings.lineBreakingRules.followingCharactersLookup.Contains(m_InternalTextElementInfo[m_CharacterCount + 1].character) == false))
                        {
                            SaveWordWrappingState(ref savedWordWrapState, i, m_CharacterCount, textInfo);
                            m_IsCharacterWrappingEnabled = false;
                            isFirstWord = false;
