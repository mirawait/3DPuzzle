...
namespace UnityEditor.TextCore
{
    ...
    public abstract class TextCoreShaderGUI : ShaderGUI
    {
        ...
        protected bool BeginPanel(string panel, bool expanded)
        {
            ...
            GUI.enabled = true;
            expanded = TextCoreEditorUtilities.EditorToggle(r, expanded, new GUIContent(panel), TM_EditorStyles.panelTitle);
            r.width -= 30;
            EditorGUI.LabelField(r, new GUIContent(expanded ? s_PanelStateLabel[0] : s_PanelStateLabel[1]), TM_EditorStyles.rightLabel);
            GUI.enabled = enabled;
        ...
        {
            Texture droppedTex = droppedMaterial.GetTexture(TextShaderUtilities.ID_MainTex);
            if (!droppedTex)
            {
                return;
            }

            Texture currentTex = m_Material.GetTexture(TextShaderUtilities.ID_MainTex);
            FontAsset requiredFontAsset = null;
            if (droppedTex != currentTex)
            {
                requiredFontAsset = TM_EditorUtilities.FindMatchingFontAsset(droppedMaterial);
