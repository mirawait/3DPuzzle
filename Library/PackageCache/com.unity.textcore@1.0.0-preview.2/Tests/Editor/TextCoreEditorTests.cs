...
{
    [Category("TextCoreBasicEditorTests")]
    public class TextCoreEditorTests
    {
        private const string m_CharacterSet_ASCII = " !\"#$%&-()*+,-./0123456789:;<=>?@abcdefghijklmnopqrstuvwxyz[-]^_`abcdefghijklmnopqrstuvwxyz{|}~-";

        private readonly string[] m_FontNames = new string[] { "Verdana", "System Normal", "NotoSans-Regular" };
        ...
        */
        ...
        public void Setup()
        {
            if (!Directory.Exists(Path.GetFullPath("Packages/com.unity.textmeshpro.tests/TextMesh Pro")))
            {
                Debug.Log("Skipping over Editor tests as test resources are missing from the current test project.");
                Assert.Ignore();
            }
        }

        // [Test]
        // public void FontAsset_GetPathsToOSFonts()
        // {
        //     string[] systemFonts = Font.GetPathsToOSFonts();
        //
        //     for (int i = 0; i < systemFonts.Length; i++)
        //         Debug.Log("[" + i + "] Font file [" + System.IO.Path.GetFileNameWithoutExtension(systemFonts[i]) + "].");
        //
        //     Assert.True(true);
        // }

        [Test]
        public void FontAsset_CreateDynamicFontAsset_From_FontObject()
        {
            ...
            {
                FontAsset fontAsset = FontAsset.CreateFontAsset(fonts[i]);

                Assert.NotNull(fontAsset);

                Assert.True(fontAsset.TryAddCharacters(m_CharacterSet_ASCII));

                for (int j = 0; j < m_CharacterSet_ASCII.Length; j++)
                {
                    Character character = FontAssetUtilities.GetCharacterFromFontAsset(m_CharacterSet_ASCII[j], fontAsset, false, FontStyles.Normal, TextFontWeight.Regular, out bool isAlternativeTypeFace);
        ...
        // [Test]
        // public void FontAsset_CreateDynamicFontAsset_From_FontNames()
        // {
        //     for (int i = 0; i < m_FontNames.Length; i++)
        //     {
        //         FontAsset fontAsset = FontAsset.CreateFontAsset(m_FontNames[i]);
        //
        //         Debug.Log("Font Asset created from font name: [" + m_FontNames[i] + "] is " + (fontAsset != null ? "Valid." : "Null."));
        //
        //         Assert.NotNull(fontAsset);
        //
        //         Assert.True(fontAsset.TryAddCharacters(m_CharacterSet_ASCII));
